...
import hu.akoel.restful.UserEntity;
...
public class TestUser {
	...
	public void deleteRecords() {
		for (UserEntity userl : userList) {
			response = service.path("rest").path("users")
					.path(userl.getId().toString()).request()
	...
	public void testPostUsers_CaptureUser() {
		...
		// Console message
		System.err.println("POST/users" + " - Status: " + response.getStatus()
	...
	public void testGetUserId_GetASpecificUser() {
		...
		// GET/users/{id}
		String result = service.path("rest").path("users")
	...
	public void testPutUsers_ModifyUser() {
		...
		// Assert values
		UserEntity resultUser = response.readEntity(UserEntity.class);
	...
	public void testGetUsers_GetList() {
		...
		// GET/users - get user list
		response = service.path("rest").path("users").request()
				.accept(MediaType.APPLICATION_XML).get();
		System.err.println("GET/users" + " - Status: " + response.getStatus()
				+ " Reason: " + response.getStatusInfo());
		System.err.println("        "
				+ service.path("rest").path("users").request()
						.accept(MediaType.APPLICATION_JSON).get(String.class));
		System.err.println();

		String result = service.path("rest").path("users").request()
				.accept(MediaType.APPLICATION_JSON).get(String.class);
	...
	private static URI getBaseURI() {
		return UriBuilder.fromUri("http://localhost:8080/Restful")
				.build();
	}
}
